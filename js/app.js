...
function heroVisualSVG() {
  return `<svg viewBox="0 0 400 500" xmlns="http://www.w3.org/2000/svg" class="hero-visual-art" preserveAspectRatio="xMidYMid meet">
    <defs>
      <linearGradient id="hgrad" x1="0" y1="0" x2="0" y2="1">
        <stop offset="0%" stop-color="#d4302a" stop-opacity="0.6"/>
        <stop offset="100%" stop-color="#1d4ed8" stop-opacity="0.6"/>
      </linearGradient>
    </defs>
    <!-- Stylised futurist car -->
    <g transform="translate(0, 180)">
      <ellipse cx="200" cy="160" rx="180" ry="14" fill="#000" opacity="0.4"/>
      <path d="M20 130 Q30 80 90 70 L140 40 Q200 25 270 30 Q330 35 360 60 L380 90 Q385 120 375 140 L370 145 L30 145 Q22 142 20 130 Z" fill="#fafafa"/>
      <path d="M100 70 L145 45 Q200 32 270 36 Q320 40 345 55 L365 85 Z" fill="url(#hgrad)" opacity="0.8"/>
      <path d="M30 130 Q40 100 90 90 L140 70 Q200 60 270 62 Q330 66 360 80 L378 110 L378 130 Z" fill="none" stroke="rgba(255,255,255,0.2)" stroke-width="1"/>
      <!-- Wheels -->
      <circle cx="100" cy="145" r="26" fill="#0a0a0a"/>
      <circle cx="100" cy="145" r="18" fill="#1a1a1a" stroke="#444" stroke-width="1"/>
      <circle cx="100" cy="145" r="6" fill="#666"/>
      <circle cx="305" cy="145" r="26" fill="#0a0a0a"/>
      <circle cx="305" cy="145" r="18" fill="#1a1a1a" stroke="#444" stroke-width="1"/>
      <circle cx="305" cy="145" r="6" fill="#666"/>
      <!-- Light strip -->
      <rect x="350" y="95" width="28" height="4" rx="2" fill="#d4302a"/>
      <rect x="22" y="95" width="28" height="4" rx="2" fill="#1d4ed8" opacity="0.8"/>
    </g>
    <!-- Top accent line -->
    <line x1="40" y1="40" x2="360" y2="40" stroke="#d4302a" stroke-width="1" opacity="0.6"/>
    <text x="40" y="32" font-family="JetBrains Mono,monospace" font-size="11" fill="rgba(255,255,255,0.6)" letter-spacing="3">EV · INTELLIGENT · MADE IN CHINA</text>
    <!-- Bottom data -->
    <text x="40" y="450" font-family="JetBrains Mono,monospace" font-size="10" fill="rgba(255,255,255,0.4)" letter-spacing="2">800V · 5C · L2++ · CTB · HARMONY</text>
  </svg>`;
}
        ...
        <div class="home-brands-col">
          ...
          <div class="home-brands-col-list">
            ${D.brands.filter(b=>b.category==='group').map(b => `
              <a href="#/brands" class="brand-cell">
        ...
        <div class="home-brands-col home-brands-col-subs">
          ...
          <div class="home-brands-col-list home-brands-col-list-compact">
            ${D.brands.filter(b=>b.category==='subbrand').map(b => `
              <a href="#/brands" class="brand-pill">${b.name}</a>
            `).join('')}
          </div>
        </div>
      </div>
...
function render() {
  ...
  // scroll to top on route change (smooth disabled for instant feel)
  window.scrollTo({ top: 0, behavior: 'instant' });
  // close mobile menu
  document.querySelector('.main-nav')?.classList.remove('open');
  // page-specific init
  if (route === '/models') initModelsFilter();
  // brand focus: ?focus=<id> on /brands → scroll to that brand card + highlight
  if (route === '/brands') {
    const q = getRouteQuery();
    if (q.focus) focusBrandCard(q.focus);
  }
}

function focusBrandCard(brandId) {
  // Wait one frame so layout settles after innerHTML swap
...
function applyI18n() {
  ...
  // 支持 data-i18n-attr="attr:key" 格式，例如 placeholder:newsletter.placeholder
...
// ============ Brand sticky nav: smooth scroll + active highlight ============
...
// Apply offset whenever a brand sticky nav is in the DOM
const _brandStickyObserver = new MutationObserver(setBrandStickyOffset);
_brandStickyObserver.observe(document.body, { childList: true, subtree: true });
...
function updateBrandStickyActive() {
  ...
  for (const id of ids) {
    const el = document.getElementById(id);
    if (!el) continue;
    if (el.getBoundingClientRect().top - sentinel <= 0) active = id;
  }
